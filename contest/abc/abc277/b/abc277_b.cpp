...
int main() {
    ...
    cin >> N;
    vector<string> S(N);
    unordered_set<char> fst = {'H', 'D', 'C', 'S'};
    unordered_set<char> snd = {'A', '2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K'};
    ...
    rep(i, N) {
        if (fst.find(S.at(i).at(0)) == fst.end()) {
            ans = false;
            break;
        }

        if (snd.find(S.at(i).at(1)) == snd.end()) {
            ans = false;
            break;
        }

        int cnt = std::count(all(S), S.at(i));
        if (cnt > 1) {
            ans = false;
            break;
        }
